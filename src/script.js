...
async function showTeachingMaterials() {
  ...
  try {
    ...
    eindString +=
      "<tr><td>ID</td><td>Name</td><td>Format</td><td>Amount</td><td>Description</td><td>Available</td></tr>";
...
async function showTeachingMaterialsPerFormat() {
  ...
  if (!teachingMaterialFormats.includes(materialFormat)) {
    ...
    const materialResponse = await fetch(
      "http://localhost:8082/teachingMaterialData"
    );
...
async function reserveFrontEnd(userId, contentId) {
  try {
    // await fetch(
    //   `https://wt2407.azurewebsites.net/reserveBook/${userId}/${contentId}`
    // );
