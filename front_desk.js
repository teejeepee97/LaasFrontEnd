...
async function reserveFrontEnd(userId, contentId) {
    try {
        await fetch(`http://localhost:8082/reserveFrontEnd/${userId}/${contentId}`);
    } catch (error) {
        console.log(error.message);
    }
}
...
async function returnBook(reservationId){
    ...
    const response = await fetch(`http://localhost:8082/returnBook/${reservationId}`);
    console.log(response);
    if (!response.ok) {
        throw new Error(`Response status: ${response.status}`);
    }
...
async function showBooksByReservationsStatus(status){
    ...
    try {
        const response = await fetch(`http://localhost:8082/showBooksByReservationStatus/${statusText}`);
        console.log(response);
        if (!response.ok) {
            throw new Error(`Response status: ${response.status}`);
        }
